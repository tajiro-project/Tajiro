...
const benefitHeader = reactive({
  locationLabel: '선호 지역',
  locationPickerRequestId: 0,
  setLocationLabel(label) {
    this.locationLabel = label || '선호 지역';
  },
  requestLocationPicker() {
    this.locationPickerRequestId += 1;
  },
...
provide('benefitHeader', benefitHeader);

const benefitLocationLabel = computed(() =>
  benefitHeader.locationLabel.trim().split(/\s+/).slice(0, 2).join(' '),
);
...
<template>
  <div class="app-frame">
    ...
    >
      ...
      <template
        v-if="route.meta.headerAction === 'preferred-region'"
        #right
      >
        <button
          class="header-region-action"
          type="button"
          :aria-label="`${benefitHeader.locationLabel} 선호 지역 변경`"
          @click="benefitHeader.requestLocationPicker"
        >
          <MapPin :size="18" :stroke-width="2" />
          <span>{{ benefitLocationLabel }}</span>
        </button>
      </template>
    </PageHeader>
...
.header-region-action svg {
  flex-shrink: 0;
}

.header-region-action span {
  overflow: hidden;
  color: var(--text-primary);
  text-overflow: ellipsis;
  white-space: nowrap;
